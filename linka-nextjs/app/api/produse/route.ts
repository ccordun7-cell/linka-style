...
export async function POST(req: NextRequest) {
  ...
  const { name, brand_name, product_code, type, category, price, description, is_barefoot, is_premium, is_school, is_sale, sale_price, sizes, images } = body
  ...
  const { data: product, error: productError } = await supabaseAdmin
    .from('products')
    .insert({ slug, name, product_code, brand_id, type, category, price, description, is_barefoot, is_premium: !!is_premium, is_school: !!is_school, is_sale: !!is_sale, sale_price: is_sale ? sale_price : null })
    .select()
    .single()
